...
export function TransferProgressBar({
  ...
  transfer: TransferProgressT
  onCancel?: () => void
}) {
  if (transfer.phase === 'idle') return null
  const pct =
    transfer.totalBytes > 0
      ? Math.min(100, Math.round((transfer.bytesSent / transfer.totalBytes) * 100))
      : 0
  ...
  return (
    <div className="rounded-2xl border border-zinc-800 bg-zinc-900/80 p-4">
      ...
      <div className="mt-2 flex items-center justify-between text-[11px] text-zinc-500">
        <span>
          chunk {transfer.chunkIndex}/{transfer.totalChunks} ·{' '}
          {(transfer.bytesSent / 1024).toFixed(1)}KB / {(transfer.totalBytes / 1024).toFixed(1)}KB
        </span>
        <span>{pct}%</span>
      </div>
      ...
      {!isDone && !isError && onCancel && (
        <button
          onClick={onCancel}
          className="mt-3 w-full rounded-lg border border-zinc-700 py-1.5 text-[11px] text-zinc-300 hover:bg-zinc-800"
        >
          Cancel Transfer
        </button>
      )}
