...
export function FaceGallery({
  ...
  const categories = Array.from(
    new Set(savedFaces.map(f => f.category).filter(Boolean))
  ) as string[]
  ...
  useEffect(() => {
    const ids = Object.entries(savedFaceStatus)
      .filter(([, v]) => v === 'sent' || v === 'error')
      .map(([k]) => k)
            ...
            <div
              className={cn(
                ...
                selectedFace === face.index
                  ? 'ring-blue-500'
                  : 'ring-transparent group-hover:ring-zinc-700'
              )}
            >
      ...
      {filteredSavedFaces.length === 0 ? (
        <div className="rounded-xl border border-dashed border-zinc-800 p-4 text-center">
          <p className="text-xs text-zinc-500">
            {savedFaces.length === 0
              ? 'No saved faces yet. Upload one to get started!'
            ...
            return (
              <div key={face.id} className="flex flex-col items-center gap-1.5">
                ...
                <div className="flex items-center gap-1">
                  <span className="max-w-[64px] truncate text-[10px] text-zinc-400">
                    {face.name}
                  </span>
                  <button
                ...
                {face.category && <span className="text-[9px] text-zinc-600">{face.category}</span>}
              </div>
            )
          })}
        </div>
