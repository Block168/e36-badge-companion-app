...
export function useBLEManager() {
  ...
  const disconnect = useCallback(() => {
    ...
    if (transferTimer.current) {
      ...
      setTransfer((t) => (t.phase === "uploading" || t.phase === "ack-wait" ? { ...t, phase: "cancelled" } : t));
    }
  }, [pushLog]);
  ...
  const runTransfer = useCallback(
    (opts: { totalBytes: number; frameIndex: number; totalFrames: number; onDone: () => void; onFail?: () => void; faceName?: string }) => {
        ...
        window.setTimeout(() => {
          ...
          transferTimer.current = window.setInterval(() => {
            if (connectionState !== "ready") {
              window.clearInterval(transferTimer.current!);
              transferTimer.current = null;
              setTransfer((t) => ({ ...t, phase: "error", error: "Peripheral disconnected mid-transfer" }));
              pushLog("xfer", "ERROR: disconnected mid-transfer, will resume from last acked chunk on reconnect");
              opts.onFail?.();
              return;
            }
            ...
            if (cursor >= totalChunks) {
              ...
              pushLog("xfer", `status notify: OK, frame ${opts.frameIndex + 1}/${opts.totalFrames} complete`);
