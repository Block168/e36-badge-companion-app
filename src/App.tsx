...
import { BluetoothConnected, Radio, Image as ImageIcon, Play, Smile, Heart } from 'lucide-react'
import { StatusPillMemo as StatusPill } from './components/StatusBar'
  ...
  return (
    <div
      className={cn(
        ...
        theme === 'dark'
          ? 'bg-[radial-gradient(ellipse_at_top,_#1e293b_0%,_#020617_60%)]'
          : 'bg-[radial-gradient(ellipse_at_top,_#f1f5f9_0%,_#ffffff_60%)] text-slate-900'
      )}
    >
      <main className="mx-auto flex min-h-screen w-full max-w-3xl flex-col px-4 py-4 sm:px-6">
        <section
          className={cn(
            'relative flex min-h-[calc(100vh-2rem)] flex-col overflow-hidden rounded-[2rem] border',
            theme === 'dark' ? 'border-zinc-900 bg-zinc-950/70' : 'border-slate-200 bg-white/80'
          )}
        >
          <div
            className={cn(
          ...
          <div className="relative flex-1 overflow-hidden">
            ...
            {tab === 'faces' && (
              <FaceGallery ble={ble} onOpenUploader={() => setUploaderOpen(true)} />
            )}
            {tab === 'brightness' && <BrightnessControl ble={ble} />}
            {tab === 'boot' && <BootAnimationEditor ble={ble} />}
            {tab === 'history' && <TransferHistory theme={theme} />}
            {uploaderOpen && (
              <CustomFaceUploader ble={ble} onClose={() => setUploaderOpen(false)} />
            )}
          </div>

          <nav
            className={cn(
              'flex items-center justify-around border-t px-1 py-2.5',
              theme === 'dark'
